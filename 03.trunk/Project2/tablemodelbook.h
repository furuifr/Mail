...
public:
     explicit TableModelBook(int rowCount=100,int pageSize=10,bool allChecked=false,QObject *parent = nullptr,QList<Contact> dataList={});
    ...
    QList<Contact> m_dataList;//显示的内容，将数据库的内容放入其中

private:

    QHash<int,int>m_pageHash;
    QList<Contact> dataList;
    int m_pageSize;
    int m_rowCount;
