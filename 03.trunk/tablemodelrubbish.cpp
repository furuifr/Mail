...
TableModelRubbish::TableModelRubbish(int rowCount,int pageSize,bool allchecked,QObject *parent,QList<Mail> dataList)
...
QVariant TableModelRubbish::headerData(int section, Qt::Orientation orientation, int role) const
{
...
int TableModelRubbish::rowCount(const QModelIndex &parent) const
{
    return m_rowCount;
}

int TableModelRubbish::columnCount(const QModelIndex &parent) const
{
   return 5;
...
QVariant TableModelRubbish::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
Qt::ItemFlags TableModelRubbish::flags(const QModelIndex &index) const{
    if(!index.isValid())
...
int TableModelRubbish::indexOfROw(int row) const{
    int page=row/m_pageSize;
    if(!m_pageHash.contains(page)){
        (const_cast<TableModelRubbish*>(this))->fetchData(page);
    }
    return m_pageHash.value(page)+row%m_pageSize;
}
bool TableModelRubbish::setData(const QModelIndex &index, const QVariant &value, int role){
    if(!index.isValid())
        return false;
...
void TableModelRubbish::fetchData(int page){
...
void TableModelRubbish::populateModel(){
